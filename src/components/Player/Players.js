...
function calculatePlayerExperience(player = {}) {
  ...
  const sinceYear = Number(String(player.since || "").trim());
...
function getPlayerMatchResultInfo(result) {
  const value = String(result || "").trim().toLowerCase();

  if (["w", "win", "won"].includes(value)) {
    return {
      label: "WIN",
      color: "#16A34A",
      background: "#F0FDF4",
...
function getPlayerMatchOpponent(match = {}) {
  if (String(match.match_type || "").toLowerCase() === "singles") {
    return String(match.opponent_name || "").trim();
  }
...
function getPlayerMatchScore(match = {}) {
  return [match.score1, match.score2, match.score3]
...
function ReportModal({ target, submitting, onClose, onSubmit }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!reason) return setError("Please choose a report reason.");
    if (reason === "Other" && !details.trim()) return setError("Please explain the report.");
    setError("");
  ...
  return (
    ...
    }} style={{ position: "fixed", inset: 0, zIndex: 3000, background: "rgba(13, 27, 62, 0.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 18 }}>
      <form onSubmit={handleSubmit} style={{ width: "min(520px, 100%)", background: C.card, border: `1px solid ${C.line}`, borderRadius: 18, padding: 20, boxShadow: "0 24px 60px rgba(13,27,62,0.25)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: C.text }}>Report {target.type}</div>
            <div style={{ marginTop: 4, fontSize: 12, color: C.muted }}>Report {target.name} to the ShuttleTrack administrator.</div>
          </div>
          <button type="button" onClick={onClose} disabled={submitting} aria-label="Close report form" style={{ width: 32, height: 32, borderRadius: 999, border: `1px solid ${C.line}`, background: C.card, color: C.muted, cursor: submitting ? "wait" : "pointer", fontSize: 18 }}>×</button>
        </div>

        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", marginBottom: 6, fontSize: 11, fontWeight: 800, color: C.muted, textTransform: "uppercase", letterSpacing: 0.7 }}>Reason</label>
          <select className={styles.formSelect} value={reason} onChange={(event) => setReason(event.target.value)} disabled={submitting} style={{ width: "100%" }}>
            {REPORT_REASON_OPTIONS.map((option) => <option key={option}>{option}</option>)}
          </select>
        </div>

        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", marginBottom: 6, fontSize: 11, fontWeight: 800, color: C.muted, textTransform: "uppercase", letterSpacing: 0.7 }}>Details</label>
          <textarea className={styles.formInput} rows={5} maxLength={1000} value={details} onChange={(event) => setDetails(event.target.value)} disabled={submitting} placeholder="Describe what happened. Do not include passwords or private financial information." style={{ width: "100%", resize: "vertical", fontFamily: "inherit" }} />
          <div style={{ marginTop: 5, textAlign: "right", fontSize: 10, color: C.muted }}>{details.length}/1000</div>
        </div>

        {error && <div style={{ marginBottom: 14, padding: 11, borderRadius: 10, background: "#FEF2F2", color: "#B91C1C", fontSize: 12 }}>{error}</div>}

        <div style={{ padding: 12, borderRadius: 11, background: "#FFF7ED", color: "#9A3412", fontSize: 11, lineHeight: 1.6, marginBottom: 16 }}>
          Reports are reviewed by an administrator. Submitting a false report may lead to account action.
        </div>

        <div style={{ display: "flex", justifyContent: "flex-end", gap: 9 }}>
          <button type="button" className={styles.btnOutline} onClick={onClose} disabled={submitting}>Cancel</button>
          <button type="submit" disabled={submitting} style={{ border: "none", borderRadius: 10, padding: "9px 15px", background: "#DC2626", color: "#FFFFFF", fontSize: 12, fontWeight: 800, cursor: submitting ? "wait" : "pointer", opacity: submitting ? 0.65 : 1 }}>{submitting ? "Submitting..." : "Submit report"}</button>
        </div>
      </form>
    </div>
  );
...
function PlayerDetail({ p, isPartner, onAddOpponent, onRemoveOpponent, onAddPartner, onCancelPartnerRequest, onRemovePartner, onAddFavourite, onRemoveFavourite, onReport }) {
  ...
  const verificationCount = verificationPlayerCount + verificationCoachCount;

  const verificationLabel =
    verificationCoachCount > 0 && verificationPlayerCount > 0
      ? `Coach + ${verificationPlayerCount} player${verificationPlayerCount === 1 ? "" : "s"}`
      : verificationCoachCount > 0
        ? "Coach verified"
        : verificationPlayerCount > 0
          ? `Verified by ${verificationPlayerCount} player${verificationPlayerCount === 1 ? "" : "s"}`
          : "Not verified";
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div className={styles.card}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}>
              ...
              {p.isFavourite && <span className={styles.badgeBlue}>Favourite</span>}
            </div>
          </div>
        </div>
        ...
        {p.ig && (
          <div style={{ marginBottom: 14 }}>
            <a href={`https://instagram.com/${p.ig.replace("@", "")}`} target="_blank" rel="noreferrer" style={{ display: "inline-flex", padding: "4px 12px", background: "#FFF0F6", border: "1px solid #FBC8DC", borderRadius: 20, textDecoration: "none" }}>
              <span style={{ fontSize: 11, color: "#B5305A", fontWeight: 600 }}>{p.ig}</span>
            </a>
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginBottom: 16 }}>
          {[
            ...
            <div key={stat.label} style={{ background: C.soft, borderRadius: 10, padding: 10, textAlign: "center" }}>
        ...
        <div style={{ borderTop: `1px solid ${C.line}`, paddingTop: 14, marginBottom: 16 }}>
          <div className={styles.cardTitle}>Latest matches</div>
          {p.latestMatches?.length > 0 ? (
            <div style={{ borderRadius: 12, background: C.soft, border: `1px solid ${C.line}`, overflow: "hidden" }}>
              {p.latestMatches.map((match, index) => {
                const resultInfo = getPlayerMatchResultInfo(match.result);
                const opponent = getPlayerMatchOpponent(match);
                const score = getPlayerMatchScore(match);
                return (
                  <div key={match.id || index} style={{ padding: "12px 14px", borderBottom: index !== p.latestMatches.length - 1 ? `1px solid ${C.line}` : "none" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, fontWeight: 700, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{opponent ? `vs ${opponent}` : "Match"}</div>
                        <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 5, marginTop: 3, fontSize: 10, color: C.muted }}>
                          {match.match_type && <><span>{match.match_type}</span><span>·</span></>}
                          <span>{formatPlayerMatchDate(match.match_date)}</span>
                        </div>
                      </div>
                      {score && <div style={{ flexShrink: 0, fontSize: 13, fontWeight: 700, color: C.text, whiteSpace: "nowrap" }}>{score}</div>}
                      <div style={{ flexShrink: 0, minWidth: 50, textAlign: "center", padding: "5px 9px", borderRadius: 999, background: resultInfo.background, border: `1px solid ${resultInfo.border}`, color: resultInfo.color, fontSize: 10, fontWeight: 700, whiteSpace: "nowrap" }}>{resultInfo.label}</div>
                    </div>
                  </div>
                );
              })}
            ...
            <div style={{ padding: "16px", borderRadius: 12, background: C.soft, border: `1px solid ${C.line}`, color: C.muted, fontSize: 12, textAlign: "center" }}>No match records yet.</div>
          )}
        </div>
            ...
            <button
              ...
              title={
                verificationCount > 0
                  ...
                  : "This skill profile has not been verified yet"
              }
              style={{
                border: "none",
                borderRadius: 999,
                ...
                fontSize: 10,
                fontWeight: 700,
                cursor: verificationCount > 0 ? "pointer" : "default",
                color:
                  verificationCoachCount > 0
                    ? "#7C3AED"
                    : verificationPlayerCount > 0
          ...
          >
            ...
            >
              <div>
                ...
                >
                  These are independent ratings submitted for {p.name}&apos;s current self-assessment.
                </div>
              </div>
            ...
            >
              {(p.verificationDetails || []).map((item, index) => {
                  ...
                  >
                    ...
                    >
                      <span
                        style={{
                          ...
                          fontWeight: 800,
                          color: accent,
                          background: isCoachVerification ? "#F3E8FF" : "#ECFDF5",
                          borderRadius: 999,
                          padding: "5px 9px",
                        }}
                      >
                    ...
                    >
                      {[
                        ...
                        ["Footwork", "footwork", p.footwork],
                        ["Drop shot", "drop_shot", p.dropShot],
                        ["Net play", "net_play", p.net],
                        ["Serve", "serve", p.serve],
                      ].map(([label, key, ownValue]) => (
                        <div
                          key={key}
                          style={{
                            padding: "9px 10px",
                        ...
                        >
                          ...
                          >
                            <span style={{ color: C.text }}>
                              Player: <strong>{Number(ownValue ?? 0)}</strong>
                            </span>
                            <span style={{ color: accent }}>
                              {roleLabel}: <strong>{Number(item[key] ?? ownValue ?? 0)}</strong>
                            </span>
                          </div>
                        </div>
                      ))}
                    ...
                    {item.feedback && (
                      <div
                        style={{
                          ...
                          padding: "10px 11px",
                          borderRadius: 10,
                          background: isCoachVerification ? "#FAF5FF" : "#F0FDF4",
                          border: `1px solid ${C.line}`,
                        }}
                      >
                        <div
      ...
      {p.isOpp && (
        <div className={styles.card}>
          ...
          <div style={{ display: "flex", justifyContent: "space-around", padding: "16px 0", textAlign: "center" }}>
            <div><div style={{ fontSize: 32, fontWeight: 800, color: "#00C48C" }}>{p.w}</div><div style={{ fontSize: 12, color: C.muted }}>Your wins</div></div>
            <div style={{ fontSize: 18, fontWeight: 700, color: C.muted, alignSelf: "center" }}>vs</div>
            <div><div style={{ fontSize: 32, fontWeight: 800, color: "#EF4444" }}>{p.l}</div><div style={{ fontSize: 12, color: C.muted }}>Your losses</div></div>
          </div>
          <div style={{ fontSize: 12, color: C.muted }}>Last played: {p.last}</div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {isPartner ? (
          <button className={styles.btnOutline} style={{ width: "100%", color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }} onClick={() => onRemovePartner(p)}>Remove partner</button>
        ) : p.partnerRequestStatus === "pending" ? (
          <button className={styles.btnOutline} style={{ width: "100%", color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }} onClick={() => onCancelPartnerRequest(p)}>Cancel request</button>
        ) : (
          <button className={styles.btnPrimary} style={{ width: "100%" }} onClick={() => onAddPartner(p)}>Request partner</button>
        )}

        {p.isOpp ? (
          <button className={styles.btnOutline} style={{ width: "100%", color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }} onClick={() => onRemoveOpponent(p)}>Remove opponent</button>
        ) : (
          <button className={styles.btnOutline} style={{ width: "100%" }} onClick={() => onAddOpponent(p)}>+ Add opponent</button>
        )}
      </div>

      {p.isFavourite ? (
        <button type="button" className={styles.btnOutline} onClick={() => onRemoveFavourite(p)} style={{ width: "100%", color: "#B45309", borderColor: "#FDE68A", background: "#FFFBEB" }}>★ Remove from favourites</button>
      ) : (
        <button type="button" className={styles.btnOutline} onClick={() => onAddFavourite(p)} style={{ width: "100%", color: "#1A5FFF", borderColor: "#BFDBFE", background: "#EFF6FF" }}>☆ Add to favourites</button>
      )}
...
function CoachStatusBadge({ status }) {
  if (status === "accepted") return <span className={styles.badgeGreen}>My coach</span>;
  if (status === "pending") return <span className={styles.badgeAmber}>Request pending</span>;
  if (status === "rejected") {
    return <span style={{ display: "inline-flex", alignItems: "center", borderRadius: 999, padding: "3px 8px", background: "#FEF2F2", color: "#DC2626", fontSize: 10, fontWeight: 700 }}>Request declined</span>;
  }
  return null;
}
...
function getCoachVenueMapEmbedUrl(venue, state) {
  const locationText = [venue?.venueName, venue?.venueAddress, state]
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      <div className={styles.card}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
          {coach.avatarUrl ? (
            <img src={coach.avatarUrl} alt={`${coach.name} profile`} style={{ width: 52, height: 52, borderRadius: "50%", objectFit: "cover", flexShrink: 0 }} />
          ) : (
            <div className={styles.av} style={{ width: 52, height: 52, fontSize: 17, flexShrink: 0 }}>{coach.init}</div>
          )}

          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 17, fontWeight: 800, color: C.text }}>{coach.name}</div>
            <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>{coach.club} · {coach.state}</div>
            {coach.headline && <div style={{ fontSize: 12, color: C.text, marginTop: 7, lineHeight: 1.5 }}>{coach.headline}</div>}
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 7 }}>
              <span className={styles.badgeBlue}>{coach.coachingLevel}</span>
              {coach.isAccepting ? <span className={styles.badgeGreen}>Accepting players</span> : <span className={styles.badgeGray}>Not accepting players</span>}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 16 }}>
          <div style={{ background: C.soft, borderRadius: 12, padding: 12, textAlign: "center" }}>
            <div style={{ fontSize: 10, color: C.muted }}>Experience</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: "#1A5FFF" }}>{coach.yearsExperience} year{coach.yearsExperience === 1 ? "" : "s"}</div>
          </div>
          <div style={{ background: C.soft, borderRadius: 12, padding: 12, textAlign: "center" }}>
            <div style={{ fontSize: 10, color: C.muted }}>Player capacity</div>
            <div style={{ fontSize: 18, fontWeight: 800, color: C.text }}>Up to {coach.maxPlayers}</div>
          </div>
        </div>

        <div style={{ borderTop: `1px solid ${C.line}`, paddingTop: 14 }}>
          <div className={styles.cardTitle}>Coaching specialties</div>
          {coach.specialties.length === 0 ? <div style={{ fontSize: 12, color: C.muted }}>No specialties added yet.</div> : (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {coach.specialties.map((specialty) => <span key={specialty} className={styles.badgeBlue}>{specialty}</span>)}
            </div>
          )}
        </div>
      </div>
      ...
      <div className={styles.card}>
        ...
        <div style={{ fontSize: 13, color: C.text, lineHeight: 1.7, whiteSpace: "pre-wrap" }}>{coach.bio || "This coach has not added a biography yet."}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 14, marginTop: 18 }}>
          ...
          <SmallInfo label="Primary training venue" value={coach.trainingVenues.find(venue => venue.isPrimary)?.venueName || coach.trainingVenues[0]?.venueName || coach.trainingVenue} />
          <SmallInfo label="Availability" value={coach.availability} />
          <SmallInfo label="Player levels" value={coach.playerLevels.join(", ")} />
          <SmallInfo label="Session types" value={coach.sessionTypes.join(", ")} />
        </div>
          ...
          <div style={{ marginTop: 20, paddingTop: 18, borderTop: `1px solid ${C.line}` }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {coach.trainingVenues.map((venue) => (
                <div key={venue.id} style={{ overflow: "hidden", borderRadius: 14, border: venue.isPrimary ? "1.5px solid #1A5FFF" : `1px solid ${C.line}`, background: C.soft }}>
                  <div style={{ padding: "12px 14px", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 800, color: C.text }}>{venue.venueName}</div>
                      {venue.venueAddress && <div style={{ marginTop: 3, fontSize: 11, color: C.muted, lineHeight: 1.5 }}>{venue.venueAddress}</div>}
                    </div>
        ...
        {requestStatus === "accepted" && coach.phone && (
          <a href={`https://wa.me/${getWhatsAppNumber(coach.phone)}?text=${encodeURIComponent(`Hi ${coach.name}, I found your coaching profile on ShuttleTrack and would like to ask about badminton coaching.`)}`} target="_blank" rel="noreferrer" className={styles.btnPrimary} style={{ marginTop: 16, width: "100%", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none" }}>WhatsApp coach</a>
        )}

        {(coach.certification || coach.certificationFileUrl || coach.relevantCertificates.length > 0) && (
          <div style={{ marginTop: 22, paddingTop: 18, borderTop: `1px solid ${C.line}` }}>
            ...
            {(coach.certification || coach.certificationFileUrl) && (
              <div role={coach.certificationFileUrl ? "button" : undefined} tabIndex={coach.certificationFileUrl ? 0 : undefined} onClick={() => { if (coach.certificationFileUrl) window.open(coach.certificationFileUrl, "_blank", "noopener,noreferrer"); }} onKeyDown={(event) => { if (coach.certificationFileUrl && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); window.open(coach.certificationFileUrl, "_blank", "noopener,noreferrer"); } }} style={{ marginTop: 10, padding: 14, borderRadius: 12, border: "1px solid #93B4F5", background: C.soft, cursor: coach.certificationFileUrl ? "pointer" : "default" }} title={coach.certificationFileUrl ? "Click to view certificate" : "No certificate file uploaded"}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                  <div style={{ minWidth: 0, flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 7, flexWrap: "wrap" }}>
                      <div style={{ fontSize: 14, fontWeight: 800, color: C.text }}>{coach.certification || "Certificate uploaded"}</div>
                      <span className={styles.badgeBlue}>Primary</span>
                    </div>
                    {coach.certificationIssuer && <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>Issued by {coach.certificationIssuer}</div>}
                  </div>
                </div>
              </div>
            )}
            ...
            {coach.relevantCertificates.length > 0 && (
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: C.muted, letterSpacing: 0.7, textTransform: "uppercase", marginBottom: 8 }}>Other relevant certificates</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {coach.relevantCertificates.map((certificate) => (
                    <div key={certificate.id} role="button" tabIndex={0} onClick={() => window.open(certificate.file_url, "_blank", "noopener,noreferrer")} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); window.open(certificate.file_url, "_blank", "noopener,noreferrer"); } }} title="Click to view certificate" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, padding: "11px 12px", borderRadius: 10, border: `1px solid ${C.line}`, background: C.soft, cursor: "pointer" }}>
                      <div style={{ minWidth: 0, flex: 1 }}>
                        <div style={{ fontSize: 12, fontWeight: 800, color: C.text }}>{certificate.certificate_name}</div>
                        {certificate.issuer && <div style={{ fontSize: 10, color: C.muted, marginTop: 3 }}>Issued by {certificate.issuer}</div>}
                      </div>
                      <span style={{ flexShrink: 0, color: "#1A5FFF", fontSize: 11, fontWeight: 700 }}>Open</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {requestStatus === "accepted" && (
          <div style={{ marginTop: 22, paddingTop: 18, borderTop: `1px solid ${C.line}` }}>
            {coach.clubMatch && (
              <div>
                <div className={styles.cardTitle}>Join your coach&apos;s club</div>
                <div style={{ fontSize: 13, color: C.text, lineHeight: 1.6, marginBottom: 12 }}>Your coach is from <strong>{coach.clubMatch.shortName ? `${coach.clubMatch.shortName} · ${coach.clubMatch.name}` : coach.clubMatch.name}</strong>. Joining the club is optional and separate from your coach relationship.</div>
                {coach.clubMembershipStatus === "accepted" ? <span className={styles.badgeGreen}>Already joined</span> : coach.clubMembershipStatus === "pending" ? (
                  <button type="button" className={styles.btnOutline} onClick={() => onCancelClubRequest(coach)} style={{ width: "100%", color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }}>Cancel club request</button>
                ) : (
                  <button type="button" className={styles.btnPrimary} onClick={() => onRequestClub(coach)} disabled={!coach.clubMatch.acceptingMembers} style={{ width: "100%", opacity: coach.clubMatch.acceptingMembers ? 1 : 0.55 }}>{coach.clubMatch.acceptingMembers ? "Request to join club" : "Club is not accepting members"}</button>
                )}
              </div>
            )}

            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: coach.clubMatch ? 16 : 0, paddingTop: coach.clubMatch ? 16 : 0, borderTop: coach.clubMatch ? `1px solid ${C.line}` : "none" }}>
              <button type="button" className={styles.btnOutline} onClick={() => onCancel(coach, true)} style={{ width: "100%", color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }}>Remove coach</button>
              <button type="button" className={styles.btnOutline} onClick={() => onReport(coach)} style={{ width: "100%", color: "#DC2626", borderColor: "#FECACA", background: "#FFF7F7" }}>Report coach</button>
            </div>
          </div>
        )}
      </div>

      {(requestStatus === null || requestStatus === "rejected" || requestStatus === "cancelled" || requestStatus === "removed") && (
        <div className={styles.card}>
          <div className={styles.cardTitle}>Request this coach</div>
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 10 }}>Add a short message about your training goal. This is optional.</div>
          <textarea className={styles.formInput} rows={4} placeholder="Example: I want to improve my footwork and match consistency." value={message} onChange={(event) => setMessage(event.target.value)} style={{ width: "100%", resize: "vertical", marginBottom: 10 }} />
          <button className={styles.btnPrimary} style={{ width: "100%", opacity: coach.isAccepting ? 1 : 0.55 }} disabled={!coach.isAccepting} onClick={() => onRequest(coach, message)}>{coach.isAccepting ? requestButtonLabel : "Coach is not accepting players"}</button>
        </div>
      )}
      ...
      {requestSentByCoach && (
        <div className={styles.card}>
          ...
          <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6 }}>{coach.name} invited you to connect as their player. Accepting gives the coach access to your synced progress, fitness and assigned training sessions.</div>
          {coach.requestMessage && <div style={{ marginTop: 10, padding: 11, borderRadius: 10, background: C.soft, color: C.text, fontSize: 12, lineHeight: 1.55, whiteSpace: "pre-wrap" }}>{coach.requestMessage}</div>}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginTop: 12 }}>
            <button type="button" className={styles.btnOutline} onClick={() => onDeclineIncoming(coach)} style={{ color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }}>Decline</button>
            <button type="button" className={styles.btnPrimary} onClick={() => onAcceptIncoming(coach)}>Accept coach</button>
          </div>
        </div>
      )}
      ...
      {requestStatus === "pending" && !requestSentByCoach && (
        <div className={styles.card}>
          ...
          <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6 }}>The coach can accept or decline this request from the coach page.</div>
          <button className={styles.btnOutline} style={{ width: "100%", marginTop: 12, color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }} onClick={() => onCancel(coach, false)}>Cancel request</button>
        </div>
      )}

      {requestStatus !== "accepted" && <button type="button" className={styles.btnOutline} onClick={() => onReport(coach)} style={{ width: "100%", color: "#DC2626", borderColor: "#FECACA", background: "#FFF7F7" }}>Report coach</button>}
    </div>
  );
}
...
export default function Players() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (userError) console.error("Failed to read current user:", userError);

      const [
        publicPlayerResult, profilePlayerResult, equipmentResult, skillResult,
        ...
        supabase.from("coach_certifications").select("*").order("sort_order", { ascending: true }).order("created_at", { ascending: true }),
        supabase.from("coach_training_venues").select("*").order("is_primary", { ascending: false }).order("sort_order", { ascending: true }).order("created_at", { ascending: true }),
        supabase.from("clubs").select("id, short_name, name, accepting_members"),
        supabase.from("club_members").select(`user_id,status,clubs (id,short_name,name)`).eq("status", "accepted"),
        supabase.from("player_matches").select(`
          id, player_id, match_type, match_date, partner_name,
          opponent_name, opponent_name2, score1, score2, score3,
          result, created_at
      ...
      if (profilePlayerResult.error) console.error("Failed to load registered player profiles:", profilePlayerResult.error);
      if (equipmentResult.error) console.error("Failed to load player equipment:", equipmentResult.error);
      if (skillResult.error) console.error("Failed to load player skills:", skillResult.error);
      if (playerMediaResult.error) console.error("Failed to load player profile media:", playerMediaResult.error);
      if (coachResult.error) console.error("Failed to load coaches:", coachResult.error);
      if (coachCertificatesResult.error) console.error("Failed to load coach certificates:", coachCertificatesResult.error);
      if (coachVenuesResult.error) console.error("Failed to load coach training venues:", coachVenuesResult.error);
      if (clubsResult.error) console.error("Failed to load clubs:", clubsResult.error);
      if (acceptedMembershipsResult.error) console.error("Failed to load accepted club memberships:", acceptedMembershipsResult.error);
      ...
      (acceptedMembershipsResult.data || []).forEach((row) => {
        ...
        const clubName = String(club?.short_name || club?.name || "").trim();
        if (clubName) acceptedClubByUserId.set(userId, clubName);
      });
      ...
      (playerMatchesResult.data || []).forEach((match) => {
        ...
        const key = String(match.player_id);
      ...
      matchesByProfileId.forEach((matches, profileId) => {
        matchStatsByProfileId.set(profileId, calculateMatchStats(matches));
      });

      const publicMatchesByPlayerId = new Map();
      const publicMatchesByPlayerName = new Map();

      (publicPlayerMatchesResult.data || []).forEach((match) => {
      ...
      publicMatchesByPlayerId.forEach(
        (matches, playerId) => {
          publicMatchesByPlayerId.set(
            playerId,
      ...
      publicMatchesByPlayerName.forEach(
        (matches, playerName) => {
          publicMatchesByPlayerName.set(
            playerName,
            sortMatchesNewestFirst(matches),
      ...
      (coachCertificatesResult.data || []).forEach((certificate) => {
        if (!certificate.coach_user_id) return;
        const current = certificatesByCoachId.get(certificate.coach_user_id) || [];
        current.push(certificate);
        certificatesByCoachId.set(certificate.coach_user_id, current);
      });

      const venuesByCoachId = new Map();
      (coachVenuesResult.data || []).forEach((venue) => {
        if (!venue.coach_user_id) return;
        const current = venuesByCoachId.get(venue.coach_user_id) || [];
        current.push({
          id: venue.id,
        ...
        venuesByCoachId.set(venue.coach_user_id, current);
      });

      const ratingsByPlayerId = new Map();
      (skillResult.data || []).forEach((rating) => {
        [rating.player_id, rating.user_id, rating.profile_id, rating.id].filter(Boolean).forEach((key) => {
      ...
      if (user) {
        const [connectionResult, relationshipResult, partnerRequestResult, clubMembershipResult] = await Promise.all([
          supabase.from("player_connections").select("*").eq("user_id", user.id),
          supabase.from("coach_player_relationships").select("*").eq("player_user_id", user.id),
          supabase.from("player_partner_requests").select("*").eq("requester_user_id", user.id),
          supabase.from("club_members").select("*").eq("user_id", user.id),
        ]);
        if (connectionResult.error) console.error("Failed to load player connections:", connectionResult.error); else connectionData = connectionResult.data || [];
        if (relationshipResult.error) console.error("Failed to load coach relationships:", relationshipResult.error); else coachRelationshipData = relationshipResult.data || [];
        if (partnerRequestResult.error) console.error("Failed to load partner requests:", partnerRequestResult.error); else outgoingPartnerRequests = partnerRequestResult.data || [];
        if (clubMembershipResult.error) console.error("Failed to load club memberships:", clubMembershipResult.error); else ownClubMemberships = clubMembershipResult.data || [];
      }

      const equipmentByPlayerId = new Map();
      (equipmentResult.data || []).forEach((equipment) => {
        if (equipment?.player_id) equipmentByPlayerId.set(String(equipment.player_id), equipment);
      });
      ...
      (verificationRequestResult.data || []).forEach((requestRow) => {
        if (requestRow.player_user_id) {
          const key = String(requestRow.player_user_id);
          if (!verificationRequestByUserId.has(key)) {
            verificationRequestByUserId.set(key, requestRow);
          }
        }

        if (requestRow.player_profile_id) {
          const key = String(requestRow.player_profile_id);
          if (!verificationRequestByProfileId.has(key)) {
            verificationRequestByProfileId.set(key, requestRow);
          }
        }
      });

      const verificationsByRequestId = new Map();

      (verificationResult.data || []).forEach((verificationRow) => {
        if (!verificationRow.request_id) return;

        const key = String(verificationRow.request_id);
        const current = verificationsByRequestId.get(key) || [];
        current.push(verificationRow);
        verificationsByRequestId.set(key, current);
      });

      const getVerificationInfo = (playerUserId, playerProfileId) => {
        const requestRow =
          (playerUserId
            ? verificationRequestByUserId.get(String(playerUserId))
            : null) ||
          (playerProfileId
            ? verificationRequestByProfileId.get(String(playerProfileId))
            : null) ||
          null;

        const details = requestRow?.id
          ? verificationsByRequestId.get(String(requestRow.id)) || []
          : [];

        return {
          details,
          playerCount: details.filter(
            (item) => item.verifier_role === "player"
          ).length,
          coachCount: details.filter(
            (item) => item.verifier_role === "coach"
          ).length,
        };
      };
      ...
      const registeredPlayers = allRegisteredProfiles
        .filter((player) => {
          ...
          if (player.user_id && !visibleDirectoryUserIds.has(String(player.user_id))) return false;
          return true;
        })
        .map((player) => {
          ...
          return {
            ...
            dropShot: Number(rating?.drop_shot ?? rating?.dropShot ?? 0),
            net: Number(rating?.net_play ?? rating?.net ?? 0),
            serve: Number(rating?.serve ?? 0),
            matches: Number(matchStats?.totalMatches ?? player.matches ?? 0),
            winRate: Number(matchStats?.winRate ?? player.win_rate ?? 0),
            streak: matchStats?.currentStreak || player.streak || "W0",
            latestMatches,
            verificationPlayerCount: verificationInfo.playerCount,
            verificationCoachCount: verificationInfo.coachCount,
      ...
      const publicPlayers = (publicPlayerResult.data || [])
        .filter((player) => {
          if (user && player.user_id === user.id) return false;
          const publicUserId = player.user_id ? String(player.user_id) : "";
          const name = String(player.name || "").trim().toLowerCase();
          if (!name) return false;
          if (publicUserId && !visibleDirectoryUserIds.has(publicUserId)) return false;
          if (publicUserId && allRegisteredUserIds.has(publicUserId)) return false;
        ...
        .map((player) => {
          ...
          const playerMatchRecords =
            publicMatchesByPlayerId.get(playerIdKey) ||
            publicMatchesByPlayerName.get(playerNameKey) ||
            [];

          const matchStats =
            playerMatchRecords.length > 0
              ? calculateMatchStats(playerMatchRecords)
              : null;
          ...
          return {
            ...
            dropShot: Number(player.drop_shot ?? player.dropShot ?? rating?.drop_shot ?? rating?.dropShot ?? 0),
            net: Number(player.net_play ?? player.net ?? rating?.net_play ?? rating?.net ?? 0),
            serve: Number(player.serve ?? rating?.serve ?? 0),
            matches: Number(matchStats?.totalMatches ?? player.matches ?? 0),
            winRate: Number(matchStats?.winRate ?? player.win_rate ?? 0),
            streak: matchStats?.currentStreak || player.streak || "W0",
            latestMatches,
      ...
      const formattedPlayers = [...registeredPlayers, ...publicPlayers].sort((a, b) => a.name.localeCompare(b.name));

      const clubsByShortName = new Map(
        (clubsResult.data || []).filter((club) => club.short_name).map((club) => [
          String(club.short_name).trim().toUpperCase(),
          { id: club.id, shortName: String(club.short_name).trim().toUpperCase(), name: club.name || "Unnamed club", acceptingMembers: club.accepting_members !== false },
        ]),
      );
      ...
      const formattedCoaches = (coachResult.data || [])
        .filter((coach) => {
          ...
          if (coach.user_id && !visibleDirectoryUserIds.has(String(coach.user_id))) return false;
          return true;
        })
        .map((coach) => {
          ...
          return {
            ...
            trainingVenues: venuesByCoachId.get(coach.user_id) || (coach.training_venue ? [{ id: `legacy-${coach.user_id}`, venueName: coach.training_venue, venueAddress: "", locationUrl: coach.training_venue_url || "", isPrimary: true }] : []),
            ...
            relevantCertificates: (certificatesByCoachId.get(coach.user_id) || []).filter((item) => item.certificate_name && item.file_url),
            ...
            requestedBy: String(relationship?.requested_by || "").trim().toLowerCase(),
      ...
      setSelected((previous) => previous ? formattedPlayers.find((player) => player.id === previous.id) || null : null);
      setSelectedCoach((previous) => previous ? formattedCoaches.find((coach) => coach.id === previous.id) || null : null);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    setTab("coach");
    setSelected(null);
    if (!notificationCoachId || coaches.length === 0) return;
    const matchingCoach = coaches.find((coach) => String(coach.userId || "") === String(notificationCoachId) || String(coach.id || "") === String(notificationCoachId));
    if (matchingCoach) {
  ...
  useEffect(() => {
    ...
    async function loadCurrentUserId() {
      ...
      if (error) return console.error("Failed to load current user for QR:", error);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "coach_player_relationships", filter: currentUserId ? `player_user_id=eq.${currentUserId}` : undefined }, () => fetchData())
      .on("postgres_changes", { event: "*", schema: "public", table: "coach_profiles" }, () => fetchData())
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "app_users" }, () => fetchData())
      .subscribe();
    const handleVisibilityUpdated = () => fetchData();
    window.addEventListener("profile-visibility-updated", handleVisibilityUpdated);
    return () => {
      window.removeEventListener("profile-visibility-updated", handleVisibilityUpdated);
  ...
  const filtered = pool.filter((player) => {
    const matchesSearch = !search || [player.name, player.club, player.state].some((value) => String(value).toLowerCase().includes(search.toLowerCase()));
    return matchesSearch && (!levelFilter || player.level === levelFilter) && (!styleFilter || player.style === styleFilter);
  });
  ...
  const coachStates = useMemo(() => [...new Set(coaches.map((coach) => coach.state).filter(Boolean))].sort(), [coaches]);
  const coachSpecialties = useMemo(() => [...new Set(coaches.flatMap((coach) => coach.specialties).filter(Boolean))].sort(), [coaches]);

  const filteredCoaches = useMemo(() => {
    const query = coachSearch.trim().toLowerCase();
    return coaches.filter((coach) => {
      const matchesSearch = !query || [coach.name, coach.club, coach.state, coach.coachingLevel, ...coach.specialties].some((value) => String(value).toLowerCase().includes(query));
  ...
  function openCoachReport(coach) {
    setReportTarget({ id: coach.id, userId: coach.userId || null, type: "coach", name: coach.name, source: "registered" });
  }
  ...
  async function submitReport({ reason, details }) {
    ...
    if (reportTarget.userId && reportTarget.userId === user.id) {
    ...
    try {
      const { error } = await supabase.from("user_reports").insert({
        ...
        description: [`Reason: ${reason}`, `Details: ${details || "No additional details provided."}`].join("\n"),
        status: "pending",
      });
      if (error) throw error;
      setReportTarget(null);
      alert(`Your ${reportTarget.type} report has been submitted for admin review.`);
    } catch (error) {
  ...
  async function addConnection(player, type) {
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) return alert("Please log in again.");
    if (!player?.id) return alert("This player could not be identified.");
    if (!["partner", "opponent", "favourite"].includes(type)) return alert("Invalid connection type.");
    try {
      const { error } = await supabase.from("player_connections").upsert({
        user_id: user.id,
        target_player_id: player.id,
        type,
        h2h_wins: type === "opponent" ? Number(player.w || 0) : 0,
        h2h_losses: type === "opponent" ? Number(player.l || 0) : 0,
        last_played: type === "opponent" && player.last && player.last !== "—" ? player.last : null,
  ...
  async function requestPartner(player, message = "") {
    ...
    if (!player.userId) return alert("This public player is not linked to a registered account, so a partner request cannot be sent.");
    if (player.userId === user.id) return alert("You cannot send a partner request to yourself.");
    const { data: ownProfile } = await supabase.from("player_profiles").select("display_name").eq("user_id", user.id).maybeSingle();
    const { error } = await supabase.from("player_partner_requests").upsert({
      requester_user_id: user.id,
      recipient_user_id: player.userId,
      requester_name: ownProfile?.display_name || user.user_metadata?.display_name || "A player",
    ...
    if (error) {
      ...
      return alert(error.message || "Failed to send partner request.");
    }
    await fetchData();
  ...
  async function cancelPartnerRequest(player) {
    if (!window.confirm(`Cancel your partner request to ${player.name}?`)) return;
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) return alert("Please log in again.");
    if (!player.userId) return alert("This player is not linked to a registered account.");
    try {
      const { data: cancelledRequest, error: cancelError } = await supabase.from("player_partner_requests").update({ status: "cancelled", responded_at: new Date().toISOString() }).eq("requester_user_id", user.id).eq("recipient_user_id", player.userId).eq("status", "pending").select("id").maybeSingle();
      if (cancelError) throw cancelError;
      if (!cancelledRequest) throw new Error("No pending partner request was found. Refresh the page and try again.");
      const { error: notificationError } = await supabase.from("notifications").delete().eq("user_id", player.userId).eq("type", "partner_request_received").eq("source_id", cancelledRequest.id);
      if (notificationError) console.warn("Partner request cancelled, but notification cleanup was blocked:", notificationError);
      setPlayers((current) => current.map((item) => item.id === player.id ? { ...item, partnerRequestStatus: null } : item));
      setSelected((current) => current?.id === player.id ? { ...current, partnerRequestStatus: null } : current);
      await fetchData();
  ...
  async function removeConnection(player, type) {
    ...
    if (!player?.id) return alert("This player could not be identified.");
    try {
      const { error } = await supabase.from("player_connections").delete().eq("user_id", user.id).eq("target_player_id", player.id).eq("type", type);
      if (error) throw error;
      await fetchData();
    } catch (error) {
  ...
  async function requestCoach(coach, message) {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return alert("Please log in first.");
    if (coach.userId === user.id) return alert("You cannot request yourself as a coach.");
    const { data: ownPlayerProfile, error: playerProfileError } = await supabase.from("player_profiles").select("id").eq("user_id", user.id).maybeSingle();
    if (playerProfileError) {
      console.error("Failed to verify player profile:", playerProfileError);
      return alert(playerProfileError.message || "Unable to verify your player profile.");
    }
    if (!ownPlayerProfile) return alert("A player profile is required before requesting a coach.");
    if (!coach.isAccepting) return alert("This coach is not accepting players right now.");
    const { error } = await supabase.rpc("request_coach_with_notification", { target_coach_user_id: coach.userId, request_message: message.trim() || null });
    if (error) {
      console.error("Failed to request coach:", error);
      return alert(error.message || "Failed to send coach request.");
    }
    await fetchData();
  ...
  async function declineIncomingCoachRequest(coach) { await respondToIncomingCoachRequest(coach, "rejected"); }
  ...
  async function cancelCoachRelationship(coach, isAccepted) {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    const { error } = await supabase.from("coach_player_relationships").update({ status: "cancelled", responded_at: null }).eq("player_user_id", user.id).eq("coach_user_id", coach.userId);
    if (error) {
      console.error("Failed to cancel coach relationship:", error);
      return alert(error.message || "Failed to update coach request.");
    }
    await fetchData();
  }
  ...
  async function requestCoachClub(coach) {
    ...
    if (!club) return alert("This coach is not linked to a registered club.");
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) return alert("Please log in again.");
    const { error } = await supabase.from("club_members").upsert({
      club_id: club.id, user_id: user.id, status: "pending", member_role: "player", requested_at: new Date().toISOString(), responded_at: null,
    }, { onConflict: "club_id,user_id" });
    if (error) {
      console.error("Failed to request club membership:", error);
      return alert(error.message || "Failed to send club request.");
    }
    await fetchData();
  ...
  async function cancelCoachClubRequest(coach) {
    ...
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    if (authError || !user) return alert("Please log in again.");
    const { error } = await supabase.from("club_members").update({ status: "cancelled", responded_at: new Date().toISOString() }).eq("club_id", club.id).eq("user_id", user.id).eq("status", "pending");
    if (error) {
  ...
  const stopQrScanner = useCallback(async () => {
    ...
    if (scanner) {
      try { if (scanner.isScanning) await scanner.stop(); } catch (error) { console.warn("Unable to stop QR scanner:", error); }
      try { await scanner.clear(); } catch (error) { console.warn("Unable to clear QR scanner:", error); }
    }
  ...
  const processScannedValue = useCallback(async (decodedText) => {
    ...
    if (currentUserId && scannedUserId === currentUserId) {
      setScanError(
        "This is your own player QR code. Scan another player's QR."
      );
      return;
    }
  ...
  const startQrScanner = useCallback(async () => {
    ...
    try {
      ...
      const readerElement = document.getElementById("shuttletrack-qr-reader");
      if (!readerElement) throw new Error("Scanner area is not ready. Please reopen the scanner.");
      const scanner = new Html5Qrcode("shuttletrack-qr-reader", { verbose: false });
      ...
      setScanError(error?.message || "Camera could not start. Allow camera permission and try again.");
      await stopQrScanner();
    } finally {
      setScannerStarting(false);
    }
  }, [cameraActive, processScannedValue, scanSuccess, scannerStarting, stopQrScanner]);
  ...
  useEffect(() => {
    return () => {
      if (scanCloseTimerRef.current) window.clearTimeout(scanCloseTimerRef.current);
      const scanner = qrScannerRef.current;
      qrScannerRef.current = null;
  ...
  if (showLoader) return <div className={styles.card}><Loader text="Loading player directory..." /></div>;
  ...
  return (
    <div className={styles.playerReadablePage}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14, marginBottom: 16, flexWrap: "wrap" }}>
        <div className={styles.tabs} style={{ marginBottom: 0, width: "fit-content", flex: "0 1 auto", minWidth: 0 }}>
          <button className={`${styles.tab} ${tab === "all" ? styles.tabActive : ""}`} onClick={() => switchTab("all")}>All players</button>
          <button className={`${styles.tab} ${tab === "partner" ? styles.tabActive : ""}`} onClick={() => switchTab("partner")}>Find partner</button>
          <button className={`${styles.tab} ${tab === "opp" ? styles.tabActive : ""}`} onClick={() => switchTab("opp")}>My opponents</button>
          <button className={`${styles.tab} ${tab === "fav" ? styles.tabActive : ""}`} onClick={() => switchTab("fav")}>My favourites</button>
      ...
      {tab !== "partner" && tab !== "coach" && (
        <div className={styles.g2}>
          <div>
            ...
            <div style={{ fontSize: 12, color: C.muted, marginBottom: 10, fontWeight: 600 }}>{filtered.length} player{filtered.length !== 1 ? "s" : ""} found</div>

            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {filtered.length === 0 && <div className={styles.card} style={{ textAlign: "center", padding: 40, color: C.muted }}>{tab === "fav" ? "You have not added any favourite players yet." : "No players match your search."}</div>}
              {filtered.map((player) => {
                  ...
                  <div key={player.id} onClick={() => setSelected(player)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 16, cursor: "pointer", background: isSelected ? C.soft : C.card, border: isSelected ? "2px solid #1A5FFF" : `1.5px solid ${C.line}` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}>
                        ...
                        {player.isOpp && <span className={styles.badgeAmber}>Opponent</span>}
                        {player.isPartner && <span className={styles.badgeGreen}>Partner</span>}
          ...
          <div>
            {!selected ? (
              ...
              <PlayerDetail
                ...
                onAddOpponent={(player) => { addConnection(player, "opponent"); setTab("opp"); }}
                onRemoveOpponent={(player) => { removeConnection(player, "opponent"); setTab("all"); }}
                onAddPartner={(player) => requestPartner(player)}
                onCancelPartnerRequest={cancelPartnerRequest}
                onRemovePartner={(player) => removeConnection(player, "partner")}
                onAddFavourite={(player) => addConnection(player, "favourite")}
                onRemoveFavourite={(player) => removeConnection(player, "favourite")}
        ...
        <div className={styles.g2}>
          <div>
            ...
            <div className={styles.card} style={{ marginTop: 12 }}>
              ...
              {savedPartners.length === 0 && <div style={{ fontSize: 13, color: C.muted }}>No saved partners yet.</div>}
              {savedPartners.map((player) => (
                <div key={player.id} className={styles.listRow}>
                  <div className={styles.av}>{player.init}</div>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{player.name}</div><div style={{ fontSize: 11, color: C.muted }}>{player.club} · {player.level} · {player.style}</div></div>
                  <button className={styles.btnOutline} style={{ fontSize: 11, padding: "5px 10px", color: "#DC2626", borderColor: "#FECACA", background: "#FEF2F2" }} onClick={() => removeConnection(player, "partner")}>Remove</button>
                </div>
              ))}
            </div>
          </div>
                ...
                <div key={player.id} style={{ display: "grid", gridTemplateColumns: "44px minmax(0,1fr) 76px 110px", gap: 12, alignItems: "center", padding: "12px 0", borderBottom: `1px solid ${C.line}` }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: 6 }}>
                      <span className={styles.badgeBlue}>{player.level}</span><span className={styles.badgeGray}>{player.style}</span>
                      {player.reasons.map((reason) => <span key={reason} className={styles.badgeAmber}>{reason}</span>)}
                    </div>
                  </div>
      ...
      {tab === "coach" && (
        <div className={styles.g2}>
          <div>
            <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
              <input className={styles.formInput} style={{ flex: 1, minWidth: 180 }} placeholder="Search coach, club, state or specialty..." value={coachSearch} onChange={(event) => setCoachSearch(event.target.value)} />
              <select className={styles.formSelect} style={{ width: 150 }} value={coachLevelFilter} onChange={(event) => setCoachLevelFilter(event.target.value)}><option value="">All coaching levels</option>{coachLevels.map((level) => <option key={level}>{level}</option>)}</select>
              <select className={styles.formSelect} style={{ width: 135 }} value={coachStateFilter} onChange={(event) => setCoachStateFilter(event.target.value)}><option value="">All states</option>{coachStates.map((state) => <option key={state}>{state}</option>)}</select>
              <select className={styles.formSelect} style={{ width: 150 }} value={coachSpecialtyFilter} onChange={(event) => setCoachSpecialtyFilter(event.target.value)}><option value="">All specialties</option>{coachSpecialties.map((specialty) => <option key={specialty}>{specialty}</option>)}</select>
            </div>

            <div style={{ fontSize: 12, color: C.muted, marginBottom: 10, fontWeight: 600 }}>{filteredCoaches.length} coach{filteredCoaches.length !== 1 ? "es" : ""} found</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {filteredCoaches.length === 0 && <div className={styles.card} style={{ textAlign: "center", padding: 40, color: C.muted }}>No coaches match your search.</div>}
                ...
                return (
                  <div key={coach.id} onClick={() => setSelectedCoach(coach)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 16, cursor: "pointer", background: isSelected ? C.soft : C.card, border: isSelected ? "2px solid #1A5FFF" : `1.5px solid ${C.line}` }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>{coach.club} · {coach.state} · {coach.yearsExperience} year{coach.yearsExperience === 1 ? "" : "s"} experience</div>
                      <div style={{ marginTop: 6, display: "flex", gap: 4, flexWrap: "wrap" }}><span className={styles.badgeBlue}>{coach.coachingLevel}</span>{coach.specialties.slice(0, 2).map((specialty) => <span key={specialty} className={styles.badgeGray}>{specialty}</span>)}</div>
                    </div>
                    {coach.requestStatus === "accepted" ? <span className={styles.badgeGreen}>My coach</span> : coach.requestStatus === "pending" ? <span className={styles.badgeAmber}>Request pending</span> : coach.isAccepting ? <span className={styles.badgeGreen}>Available</span> : <span className={styles.badgeGray}>Unavailable</span>}
                  </div>
                );
              })}
            </div>
      ...
      {showMyQr && (
        <div role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowMyQr(false); }} style={{ position: "fixed", inset: 0, zIndex: 3200, background: "rgba(13, 27, 62, 0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: 18 }}>
          <div style={{ width: "min(420px, 100%)", background: C.card, borderRadius: 20, padding: 24, textAlign: "center", boxShadow: "0 24px 60px rgba(13,27,62,0.28)" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, marginBottom: 18, textAlign: "left" }}>
      ...
      {showScanner && (
        <div role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeScanner(); }} style={{ position: "fixed", inset: 0, zIndex: 3200, background: "rgba(13, 27, 62, 0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: 18 }}>
          <div style={{ width: "min(520px, 100%)", maxHeight: "calc(100vh - 36px)", overflowY: "auto", background: C.card, borderRadius: 20, padding: 20, boxShadow: "0 24px 60px rgba(13,27,62,0.28)" }}>
            ...
            <div style={{ position: "relative", overflow: "hidden", borderRadius: 18, border: scanSuccess ? "4px solid #16A34A" : scanError ? "3px solid #EF4444" : "3px solid #D9E2F2", background: scanSuccess ? "#F0FDF4" : "#0F172A", minHeight: 300, transition: "border-color 0.2s ease, background 0.2s ease", boxShadow: scanSuccess ? "0 0 0 6px rgba(22,163,74,0.13)" : "none" }}>
              <div id="shuttletrack-qr-reader" style={{ width: "100%", minHeight: 300, background: "#0F172A" }} />
              {!cameraActive && !scannerStarting && !scanSuccess && <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: 24, textAlign: "center", background: "linear-gradient(180deg,#172554,#0F172A)", color: "#FFFFFF" }}><div style={{ fontSize: 38 }}>📷</div><div style={{ fontSize: 13, lineHeight: 1.6, opacity: 0.85 }}>Camera did not start automatically. Press the button below and allow camera permission.</div><button type="button" className={styles.btnPrimary} onClick={startQrScanner}>Start camera</button></div>}
              {scannerStarting && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(15,23,42,0.8)", color: "#FFFFFF", fontSize: 13, fontWeight: 800 }}>Starting camera...</div>}
              {scanSuccess && <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, background: "rgba(240,253,244,0.94)", color: "#166534", fontWeight: 900 }}><div style={{ width: 70, height: 70, borderRadius: 999, display: "flex", alignItems: "center", justifyContent: "center", background: "#16A34A", color: "#FFFFFF", fontSize: 38 }}>✓</div>{scanSuccessLabel || "QR found"}</div>}
            </div>
      ...
      <ReportModal target={reportTarget} submitting={submittingReport} onClose={() => { if (!submittingReport) setReportTarget(null); }} onSubmit={submitReport} />
    </div>
  );
}
